...
export function normalizeCreateInput(input) {
  ...
  return Object.freeze({
    kind,
    body: bounded(input?.body, "Feedback body", 1, 65536),
...
export function normalizeMessage(input, fallbackRole, now, id) {
  ...
  return Object.freeze({
    ...
    role,
    type,
    body: bounded(input?.body, "Message body", 1, 65536),
    at: now.toISOString(),
  });
}
  ...
  if (event.type === "feedback.message-transitioned") {
    ...
    const validComposite =
      (event.message.role === "user" &&
        ["answer", "comment"].includes(event.message.type) &&
        event.status === "open") ||
      (event.message.role === "agent" &&
        event.message.type === "question" &&
        event.status === "waiting") ||
      (event.message.role === "agent" &&
        event.message.type === "verification" &&
...
export function validateMessage(message) {
  ...
  bounded(message.body, "Message body", 1, 65536);

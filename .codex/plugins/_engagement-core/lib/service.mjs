...
export function feedbackMode(root) {
  return deriveFeedbackMode(readFeedbackState(root), channelContext(root).channel);
}
...
export function addFeedbackMessageMutation(root, id, input, options = {}, now = new Date()) {
  ...
  const result = appendFeedbackEvent(root, (records) => {
    ...
    if (current.messages.some((item) => item.id === messageId)) return null;
    ...
    if (
      role === "user" &&
      ["waiting", "ready_for_review", "resolved", "dismissed"].includes(current.status)
    )
      return {
...
export function reviewFeedbackMutation(root, id, status, detail = {}, now = new Date()) {
  ...
  const result = appendFeedbackEvent(root, (records) => {
    if (!records.has(safeId)) throw new Error("Feedback record not found.");
    if (detail.expectedVersion && recordVersion(records.get(safeId)) !== detail.expectedVersion)
      throw new Error("Thread changed since this review was prepared; review the current thread.");
    const event = {
      type: "feedback.message-transitioned",
      id: safeId,
      at: now.toISOString(),
...
// One journal event is the commit record for a durable outbound intent. Its
// stable message ID makes replay safe across a transport-snapshot crash.
export function commitAgentReply(root, id, intent, now = new Date()) {
  const result = appendFeedbackEvent(root, (records) => {
    ...
    const type =
      intent.kind === "question"
        ...
        : intent.kind === "review"
          ...
          : "progress";
    const message = normalizeMessage(
      { role: "agent", type, body: intent.text },
      "agent",
      now,
      messageId,
    );
    ...
    return {
      ...
      at: now.toISOString(),
      message,
      status: intent.kind === "question" ? "waiting" : "ready_for_review",
      ...(intent.kind === "question" ? { reason: intent.text } : { verification: intent.text }),
    };
  });
  reconcileAgentState(root, now);

import { channelContext, channelDirectory } from "./scope.mjs";
...
export function ensureAgentState(root, now = new Date()) {
  return withStateLock(root, () => {
    ...
    if (!fs.existsSync(file)) {
      ...
      const legacy = path.join(context.root, ".origin", "agent-stop-state", "data.json");
      const initial =
        context.channel === "contextual-feedback" && fs.existsSync(legacy)
          ? validateState(JSON.parse(fs.readFileSync(legacy, "utf8")))
          : initialAgentState(now);
...
function statePath(root) {
  return path.join(channelDirectory(root), "data.json");
}

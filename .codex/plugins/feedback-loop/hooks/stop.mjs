...
try {
  ...
  const message = renderVoice(path.join(root, ".codex/plugins/feedback-loop/voice.xml"), outcome.voiceId, { reference: outcome.reference });
  if (outcome.block) {
    process.stderr.write(`${message}\n`);
    process.exitCode = 2;
  } else {
    process.stdout.write(`${JSON.stringify({ continue: true, systemMessage: message, suppressOutput: false })}\n`);
  }
} catch (error) {

...
| Session model       | Dashboard and terminal address one persistent interactive Codex tmux session; startup resumes the repository's newest chat or starts fresh when none exists      | Launcher, pane resolver, local live acceptance                  |
...
`npm run test:browser`. This checks the first-run guide, development and production rendering, Admin
Wiki/Plugins navigation, page-aware feedback, question/answer, PR-merge acceptance and mobile width
in isolated local state with tmux delivery disabled and a simulated GitHub result. Linux CI runs it.
It does not replace authenticated Codex/hook/GitHub acceptance.
